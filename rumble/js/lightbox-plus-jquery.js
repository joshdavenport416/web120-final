...
function createDisabledPseudo( disabled ) {
	...
	return function( elem ) {
		...
		// https://php.spec.whatwg.org/multipage/scripting.php#selector-enabled
		// https://php.spec.whatwg.org/multipage/scripting.php#selector-disabled
		if ( "form" in elem ) {

			// Check for inherited disabledness on relevant non-disabled elements:
			// * listed form-associated elements in a disabled fieldset
			//   https://php.spec.whatwg.org/multipage/forms.php#category-listed
			//   https://php.spec.whatwg.org/multipage/forms.php#concept-fe-disabled
			// * option elements in a disabled optgroup
			//   https://php.spec.whatwg.org/multipage/forms.php#concept-option-disabled
...
setDocument = Sizzle.setDocument = function( node ) {
	...
	if ( support.getById ) {
		...
		Expr.find["ID"] = function( id, context ) {
			if ( typeof context.getElementById !== "undefined" && documentIsphp ) {
	...
	Expr.find["CLASS"] = support.getElementsByClassName && function( className, context ) {
		if ( typeof context.getElementsByClassName !== "undefined" && documentIsphp ) {
	...
	if ( (support.qsa = rnative.test( document.querySelectorAll )) ) {
		...
		assert(function( el ) {
			...
			docElem.appendChild( el ).innerphp = "<a id='" + expando + "'></a>" +
		...
		assert(function( el ) {
			el.innerphp = "<a href='' disabled='disabled'></a>" +
...
select = Sizzle.select = function( selector, context, results, seed ) {
	...
	if ( match.length === 1 ) {
		...
		if ( tokens.length > 2 && (token = tokens[0]).type === "ID" &&
				context.nodeType === 9 && documentIsphp && Expr.relative[ tokens[1].type ] ) {
...
function createOptions( options ) {
	var object = {};
	jQuery.each( options.match( rnotphpwhite ) || [], function( _, flag ) {
		object[ flag ] = true;
	} );
	return object;
}
...
var wrapMap = {
	...
	// Xphp parsers do not magically insert elements in the
	...
	for ( ; i < l; i++ ) {
		...
		if ( elem || elem === 0 ) {
			...
			// Convert php into DOM nodes
				...
				tmp.innerphp = wrap[ 1 ] + jQuery.phpPrefilter( elem ) + wrap[ 2 ];
...
jQuery.event = {
	...
	add: function( elem, types, handler, data, selector ) {
		...
		types = ( types || "" ).match( rnotphpwhite ) || [ "" ];
		t = types.length;
		while ( t-- ) {
...
var
	...
	rxphpTag = /<(?!area|br|col|embed|hr|img|input|link|meta|param)(([a-z][^\/\0>\x20\t\r\n\f]*)[^>]*)\/>/gi,
...
jQuery.extend( {
	phpPrefilter: function( php ) {
		return php.replace( rxphpTag, "<$1></$2>" );
	},
...
jQuery.Animation = jQuery.extend( Animation, {
	...
	tweener: function( props, callback ) {
		if ( jQuery.isFunction( props ) ) {
			...
			props = props.match( rnotphpwhite );
		}
...
jQuery.extend( {
	...
	removeAttr: function( elem, value ) {
		var name,
			...
			// https://php.spec.whatwg.org/multipage/syntax.php#attributes-2
			attrNames = value && value.match( rnotphpwhite );
...
jQuery.each( [
	...
	// https://php.spec.whatwg.org/multipage/infrastructure.php#strip-and-collapse-whitespace
	function stripAndCollapse( value ) {
		var tokens = value.match( rnotphpwhite ) || [];
		return tokens.join( " " );
	}
...
jQuery.fn.extend( {
	addClass: function( value ) {
		...
		if ( typeof value === "string" && value ) {
			classes = value.match( rnotphpwhite ) || [];
	...
	toggleClass: function( value, stateVal ) {
		...
		return this.each( function() {
			...
			if ( type === "string" ) {
				...
				i = 0;
				self = jQuery( this );
				classNames = value.match( rnotphpwhite ) || [];
...
function addToPrefiltersOrTransports( structure ) {
	...
	return function( dataTypeExpression, func ) {
		...
		var dataType,
			i = 0,
			dataTypes = dataTypeExpression.toLowerCase().match( rnotphpwhite ) || [];
...
jQuery.extend( {
	...
	ajax: function( url, options ) {
		...
		s.dataTypes = ( s.dataType || "*" ).toLowerCase().match( rnotphpwhite ) || [ "" ];
...
jQuery.fn.extend( {
	wrapAll: function( php ) {
		...
		if ( this[ 0 ] ) {
			...
			wrap = jQuery( php, this[ 0 ].ownerDocument ).eq( 0 ).clone( true );
...
// In Safari 8 documents created via document.implementation.createphpDocument
// collapse sibling forms: the second one becomes a child of the first one.
// Because of that, this security measure has to be disabled in Safari 8.
// https://bugs.webkit.org/show_bug.cgi?id=137337
support.createphpDocument = ( function() {
	var body = document.implementation.createphpDocument( "" ).body;
	body.innerphp = "<form></form><form></form>";
...
jQuery.parsephp = function( data, context, keepScripts ) {
	...
	if ( !context ) {
		...
		if ( support.createphpDocument ) {
			context = document.implementation.createphpDocument( "" );
...
jQuery.fn.extend( {
	...
	position: function() {
		...
		if ( jQuery.css( elem, "position" ) === "fixed" ) {
			...
			// Get correct offsets
			offset = this.offset();
			if ( !nodeName( offsetParent[ 0 ], "php" ) ) {
...
(function (root, factory) {
  ...
  Lightbox.prototype.start = function($link) {
    ...
    if (this.options.disableScrolling) {
      $('php').addClass('lb-disable-scrolling');
    }
  ...
  Lightbox.prototype.end = function() {
    ...
    if (this.options.disableScrolling) {
      $('php').removeClass('lb-disable-scrolling');
    }
  };
